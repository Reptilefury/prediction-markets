...
/**
 * Small helper component that reads Pub/Sub-related properties and returns trimmed values to avoid
 * issues caused by accidental whitespace in env vars.
 */
@Component
public class GcpPubSubProperties {
  ...
  public GcpPubSubProperties(
      ...
      @Value("${gcp.pubsub.topics.universal-deposit-wallet:universal-deposit-wallet}")
          String topicUniversalDepositWallet,
      @Value(
              "${gcp.pubsub.subscriptions.universal-deposit-wallet:universal-deposit-wallet-subscription}")
          String subscriptionUniversalDepositWallet) {
