...
public class QrCodeAvatarService {
    ...
    public String generateWalletQrCode(String address, String label) {
        ...
        return String.format("https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=%s", 
    ...
    public String generateBitcoinDepositQrCodes(String depositAddressesJson) {
        try {
            if (depositAddressesJson != null) {
                ...
                if (bitcoinNode != null) {
                    String legacy = bitcoinNode.has("legacy_address") ? 
                            generateWalletQrCode(bitcoinNode.get("legacy_address").asText(), "BTC-Legacy") : null;
                    String segwit = bitcoinNode.has("segwit_address") ? 
                            generateWalletQrCode(bitcoinNode.get("segwit_address").asText(), "BTC-Segwit") : null;
                    String nativeSegwit = bitcoinNode.has("native_segwit_address") ? 
                            generateWalletQrCode(bitcoinNode.get("native_segwit_address").asText(), "BTC-Native") : null;
                    String taproot = bitcoinNode.has("taproot_address") ? 
                            generateWalletQrCode(bitcoinNode.get("taproot_address").asText(), "BTC-Taproot") : null;
                    
                    return String.format("{\"legacy\":\"%s\",\"segwit\":\"%s\",\"native_segwit\":\"%s\",\"taproot\":\"%s\"}", 
    ...
    public String generateDepositQrCodesJson(Object depositAddresses) {
        ...
        try {
            if (depositAddresses != null) {
                return String.format("{\"generated_at\":\"%s\",\"qr_codes\":\"available\"}", 
