...
/**
 * Global exception handler for Oregon Markets Maps exceptions to standardized response codes and
 * formats
 */
@Slf4j
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleKeycloakAuth(KeycloakAuthException ex) {
    ...
    ApiResponse<Void> response =
        ApiResponse.error(ResponseCode.KEYCLOAK_AUTH_FAILED, ex.getMessage());
    response.getError().setTraceId(traceId);
  ...
  public ResponseEntity<ApiResponse<Void>> handleWebExchangeValidation(
  ...
  public ResponseEntity<ApiResponse<Void>> handleDuplicateKeyException(DuplicateKeyException ex) {
    ...
    ApiResponse<Void> response = ApiResponse.error(ResponseCode.DUPLICATE_USER, errorMessage);
    response.getError().setTraceId(traceId);
  ...
  public ResponseEntity<ApiResponse<Void>> handleDataAccessException(DataAccessException ex) {
    ...
    ApiResponse<Void> response =
        ApiResponse.error(
            ResponseCode.DATABASE_ERROR, "A database error occurred. Please try again.");
    response.getError().setTraceId(traceId);
