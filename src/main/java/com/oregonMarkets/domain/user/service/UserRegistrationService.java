...
public class UserRegistrationService implements IUserRegistrationService {
  ...
  private Mono<User> setupExternalIntegrationsAsyncViawEvents(
    ...
    log.info(
        "[USER-REGISTRATION] Setting up external integrations for user: {}",
        DataMaskingUtil.maskUserId(user.getId().toString()));
    log.info(
        "[USER-REGISTRATION] Magic wallet address: {}",
        DataMaskingUtil.maskWalletAddress(user.getMagicWalletAddress()));
    return proxyWalletService
        ...
        .map(
            walletData -> {
              ...
              user.setBiconomySmartAccountAddress(
              ...
              log.info(
                  "[USER-REGISTRATION] Smart Account: {}",
                  DataMaskingUtil.maskWalletAddress(
                      walletData.getSmartAccount().getSmartAccountAddress()));
              log.info(
                  "[USER-REGISTRATION] Deployed: {}", walletData.getSmartAccount().getDeployed());
        ...
        .doOnError(
            error -> {
              log.error(
                  "[USER-REGISTRATION] ✗ CRITICAL: Smart account creation failed - user registration will be aborted");
              log.error(
                  "[USER-REGISTRATION] User ID: {}",
                  DataMaskingUtil.maskUserId(user.getId().toString()));
              log.error(
                  "[USER-REGISTRATION] Email: {}", DataMaskingUtil.maskEmail(user.getEmail()));
              log.error(
                  "[USER-REGISTRATION] Magic Wallet: {}",
                  DataMaskingUtil.maskWalletAddress(user.getMagicWalletAddress()));
