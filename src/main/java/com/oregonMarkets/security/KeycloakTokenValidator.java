...
/**
 * Minimal Keycloak token validator.
 * NOTE: This does not perform signature verification. For production, replace
 * with proper JWT verification against the realm's JWKS or introspection.
 */
@Component
public class KeycloakTokenValidator {
    ...
    public Mono<Map<String, Object>> validate(String bearerToken) {
        ...
        try {
            ...
            if (payload.has("preferred_username")) userInfo.put("preferred_username", payload.get("preferred_username").asText());
