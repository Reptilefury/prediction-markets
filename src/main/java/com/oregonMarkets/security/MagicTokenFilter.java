...
public class MagicTokenFilter implements WebFilter {
  ...
  private Mono<Void> validateAndProceed(ServerWebExchange exchange, WebFilterChain chain) {
    ...
    String path = exchange.getRequest().getPath().value();
    log.info(
        "MagicTokenFilter: Extracted token for path {}: {}",
        path,
    ...
    return magicValidator
        ...
        .flatMap(
            userInfo -> {
              log.info(
                  "MagicTokenFilter: Magic DID token validation successful for user: {} on path: {}",
                  userInfo.getEmail(),
                  path);
        ...
        .onErrorResume(
            e -> {
              log.error(
                  "MagicTokenFilter: Magic DID token validation failed for path {}: {}",
                  path,
                  e.getMessage(),
                  e);
  ...
  private String extractToken(ServerWebExchange exchange) {
    ...
    if (authHeader != null) {
      log.debug(
          "Authorization header length: {}, First 50 chars: {}",
          authHeader.length(),
    ...
    log.debug(
        "Extracted token length: {}, First 50 chars: {}",
        token.length(),
  ...
  private Mono<Void> sendErrorResponse(ServerWebExchange exchange, String message) {
    log.warn(
        "Sending error response: {} for path: {}",
        message,
