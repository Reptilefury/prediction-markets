...
public class Web3AuthService {
  ...
  private static final String EXPECTED_MESSAGE_PREFIX =
  ...
  public Mono<Boolean> verifySignature(String walletAddress, String message, String signature) {
    return Mono.fromCallable(
            () -> {
              ...
              boolean isValid =
                  signature != null && signature.startsWith("0x") && signature.length() == 132;
        ...
        .doOnError(
            error ->
                log.error(
                    "Error verifying signature for wallet {}: {}",
                    walletAddress,
                    error.getMessage()));
  }
  ...
  public String generateAuthMessage(String walletAddress) {
    ...
    return String.format(
        "%s%n%nWallet: %s%nTimestamp: %d", EXPECTED_MESSAGE_PREFIX, walletAddress, timestamp);
  }
}
