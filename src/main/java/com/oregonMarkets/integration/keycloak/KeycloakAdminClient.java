...
public class KeycloakAdminClient {
        ...
        return WebClient.create(baseUrl)
                ...
                .onStatus(status -> !status.is2xxSuccessful(),
                        clientResponse -> clientResponse.bodyToMono(String.class)
                                .flatMap(body -> {
                                    log.error("Keycloak token request failed - Status: {}, Response: {}",
                                            clientResponse.statusCode(), body);
                                    return Mono.error(new RuntimeException("Failed to get Keycloak access token: " +
    ...
    private Mono<Void> createUser(String username, String email, String accessToken) {
        ...
        return webClient.post()
                ...
                .bodyValue(payload)
                .retrieve()
                .onStatus(status -> !status.is2xxSuccessful() && status.value() != 409,
                        clientResponse -> clientResponse.bodyToMono(String.class)
                                .flatMap(body -> {
                                    log.error("Keycloak createUser failed for {} - Status: {}, Response: {}",
                                            username, clientResponse.statusCode(), body);
                                    return Mono.error(new RuntimeException("Failed to create Keycloak user: " +
                                            clientResponse.statusCode()));
                                }))
                .toBodilessEntity()
                .then()
    ...
    public Mono<Void> setPassword(String username, String password, String accessToken) {
        ...
        return findUserIdByUsername(username, accessToken)
                ...
                .doOnError(e -> log.warn("Failed to update Keycloak password for {}: {}", username, e.getMessage()));
    }
    ...
    private Mono<String> findUserIdByUsername(String username, String accessToken) {
        ...
        return webClient.get()
                ...
                .onStatus(status -> !status.is2xxSuccessful(),
                        clientResponse -> clientResponse.bodyToMono(String.class)
                                .flatMap(body -> {
                                    log.error("Keycloak findUserIdByUsername failed - Status: {}, Response: {}",
                                            clientResponse.statusCode(), body);
                                    return Mono.error(new RuntimeException("Failed to find user in Keycloak: " +
                                            clientResponse.statusCode()));
                                }))
