 ...
 *
 * Leverages Polymarket's ProxyWalletFactory contract on Polygon:
 * - Address: 0xaB45c5A4B0c941a2F231C04C3f49182e1A254052
 *
 * Key Architecture Notes:
 * - Wallet creation is IMPLICIT/AUTOMATIC via the `proxy()` function
 * - Wallet address is deterministic: keccak256(abi.encodePacked(userEOA))
 * - Wallets are cloned from an implementation contract
 * - Uses GSN (Gas Station Network) for gasless relayed calls
 * - No events are emitted by the factory
 *
 * Flow:
 * 1. User registers -> we calculate their deterministic proxy wallet address
 * 2. Store proxy wallet address in database
 * 3. On first proxy() call (trading), wallet is auto-created by Polymarket factory
 */
@Service
...
public class ProxyWalletOnboardingService {
     ...
     *
     * Polymarket factory creates wallets at deterministic addresses using:
    ...
    public Mono<String> createUserProxyWallet(String userEOA) {
        return Mono.fromCallable(() -> {
            ...
            String normalizedEOA = userEOA.toLowerCase().startsWith("0x")
                ? userEOA.toLowerCase()
            ...
            try {
                ...
                log.info("Calculated proxy wallet address {} for user EOA {} - wallet will be auto-created on first proxy() call",
                    proxyWalletAddress, normalizedEOA);
     ...
     *
     * Implements: keccak256(abi.encodePacked(userEOA))
     * This is the address where Polymarket's factory will deploy a new proxy wallet clone
     *
    ...
    private String calculateProxyWalletAddress(String userEOA) {
        try {
            ...
            String proxyAddress = "0x" + Numeric.toHexStringNoPrefixZeroPadded(
                new java.math.BigInteger(1, walletAddressBytes), 40);

            log.debug("Calculated deterministic proxy wallet address: {} for user: {}", proxyAddress, userEOA);
            return proxyAddress;
