...
/**
 * Client for interacting with the crypto-service API Handles wallet creation, smart account
 * management, and blockchain operations
 */
@Service
...
public class CryptoServiceClient {
   ...
   */
  public Mono<WalletCreateResponseData> createSmartAccount(String walletAddress, String didToken) {
    ...
    log.info(
        "[CRYPTO-SERVICE] Request payload: walletAddress={}",
    ...
    return webClient
        ...
        .doOnNext(
            response -> {
              ...
              log.debug(
                  "[CRYPTO-SERVICE] Response type: {}",
                  DataMaskingUtil.maskResponseObject(response));
            })
        ...
        .retryWhen(
            Retry.backoff(2, Duration.ofSeconds(1))
                ...
                .doBeforeRetry(
                    retrySignal ->
                        log.warn(
                            "[CRYPTO-SERVICE] Retrying smart account creation (attempt {}/2): {}",
        ...
        .doOnSuccess(
            response -> {
              ...
              log.info(
                  "[CRYPTO-SERVICE] Smart Account Address: {}",
        ...
        .doOnError(
            error ->
                log.error(
                    "[CRYPTO-SERVICE] ✗ Failed to create smart account for wallet {}: {}",
                    DataMaskingUtil.maskWalletAddress(walletAddress),
                    error.getMessage(),
                    error))
        .onErrorMap(
            throwable -> {
              ...
              log.error(
                  "[CRYPTO-SERVICE] Mapping error to ExternalServiceException: {}",
