...
public class MagicClient {
    ...
    @PostConstruct
    public void init() {
        log.info("Magic API Key loaded: {}... (length: {})", 
    ...
    public Mono<MagicUserInfo> validateDIDToken(String didToken) {
        // Log API key details
        log.info("API Key check - starts with sk_: {}, length: {}", 
        ...
        log.info("Headers: X-Magic-Secret-Key: {}..., User-Agent: OregonMarkets/1.0", 
                apiKey != null ? apiKey.substring(0, Math.min(8, apiKey.length())) : "null");
        ...
        return webClient.post()
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                        response -> response.bodyToMono(String.class)
                                .doOnNext(body -> log.error("Magic API error response: Status={}, Body={}", response.statusCode(), body))
                                .then(Mono.error(new MagicAuthException("Magic API returned " + response.statusCode()))))
                ...
                .onErrorMap(error -> {
                    ...
                    return new MagicAuthException("Invalid or expired DID token " + error.getMessage(), error);
                });
    }
