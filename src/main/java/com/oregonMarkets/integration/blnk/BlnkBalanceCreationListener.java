...
/**
 * Listens for EnclaveUdaCreatedEvent and asynchronously creates Blnk identity and balance Once Blnk
 * balance is created, publishes BlnkBalanceCreatedEvent to trigger Keycloak provisioning
 */
@Component
    ...
    userRepository
        ...
        .flatMap(
            user -> {
              ...
              if (user.getBlnkIdentityId() != null) {
                log.info(
                    "User {} has existing Blnk identity: {}, checking balances",
                    event.getUserId(),
                    user.getBlnkIdentityId());

                // Query Blnk for existing balances
                return blnkClient
                    .getBalancesByIdentity(user.getBlnkIdentityId())
                    .flatMap(
                        balances -> {
                          // Check if USDC balance exists
                          boolean hasUsdcBalance =
                              balances.stream()
                                  .anyMatch(balance -> "USDC".equals(balance.get("currency")));

                          if (hasUsdcBalance) {
                            log.info(
                                "User {} already has USDC balance, skipping creation",
                                event.getUserId());
                ...
                log.info(
                    "User {} has no Blnk identity, creating identity and USDC balance",
                    event.getUserId());
                ...
                return blnkClient
                    ...
                    .flatMap(
                        identityId -> {
                          log.info(
                              "Created Blnk identity for user: {}, identityId: {}",
                              event.getUserId(),
                              identityId);
        ...
        .subscribe(
            blnkDetails -> {
              if (blnkDetails != null) {
                log.info(
                    "Blnk USDC balance ready for user: {}, identityId: {}, balanceId: {}",
                    event.getUserId(),
                    blnkDetails.identityId,
                    blnkDetails.balanceId);

                // Publish BlnkBalanceCreatedEvent
                BlnkBalanceCreatedEvent blnkEvent =
            ...
            error -> {
              log.error(
                  "Failed to process Blnk balance for user {}: {}",
                  event.getUserId(),
                  error.getMessage(),
                  error);
            });
