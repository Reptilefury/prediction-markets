...
class KeycloakTokenFilterTest {
    ...
    void filter_InvalidKeycloakToken_ReturnsUnauthorized() {
        MockServerHttpRequest request = MockServerHttpRequest.get("/api/users")
            .header("X-Keycloak-Token", "invalid-token")
            .build();
        MockServerWebExchange exchange = MockServerWebExchange.from(request);
        
        when(validator.validate("invalid-token")).thenReturn(Mono.error(new RuntimeException("Invalid token")));
    ...
    void filter_TokenWithoutBearerPrefix_ProcessesCorrectly() {
        MockServerHttpRequest request = MockServerHttpRequest.get("/api/users")
            .header("X-Keycloak-Token", "raw-token")
            .build();
        MockServerWebExchange exchange = MockServerWebExchange.from(request);
