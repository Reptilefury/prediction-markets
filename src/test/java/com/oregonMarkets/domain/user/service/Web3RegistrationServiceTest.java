...
class Web3RegistrationServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new Web3RegistrationService(userRepository, web3AuthService, enclaveClient, cacheService);
        ReflectionTestUtils.setField(service, "destinationTokenAddress", "0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174");
    }
    ...
    void registerUser_Success() {
        ...
        when(userRepository.existsByWeb3WalletAddress("0x123"))
        ...
        when(cacheService.set(anyString(), any(), any(Duration.class)))
    ...
    void registerUser_WithReferralCode() {
        ...
        when(userRepository.existsByWeb3WalletAddress("0x123"))
            .thenReturn(Mono.just(false));
        when(userRepository.findByReferralCode("REF123"))
            .thenReturn(Mono.just(referrer));
        when(enclaveClient.createUDA(anyString(), anyString(), anyString(), anyString()))
            .thenReturn(Mono.just(udaResponse));
        when(userRepository.save(any(User.class)))
            .thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
