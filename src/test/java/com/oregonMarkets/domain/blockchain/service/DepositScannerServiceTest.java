...
class DepositScannerServiceTest {
  ...
  void setUp() {
    depositScannerService =
        new DepositScannerService(chainRepository, addressRepository, depositRepository);
  }
  ...
  void scanChainForDeposits_ChainNotFound_CompletesEmpty() {
    ...
    StepVerifier.create(depositScannerService.scanChainForDeposits(chainId)).verifyComplete();
  }

  @Test
  void scanChainForDeposits_ChainInactive_CompletesEmpty() {
    UUID chainId = UUID.randomUUID();
    BlockchainChain inactiveChain =
        BlockchainChain.builder().id(chainId).chainName("ethereum").isActive(false).build();
  ...
  void scanChainForDeposits_ActiveChain_ProcessesSuccessfully() {
    ...
    StepVerifier.create(depositScannerService.scanChainForDeposits(chainId)).expectError().verify();
  }
  ...
  void scanChainForDeposits_NullChainId_CompletesEmpty() {
    ...
    StepVerifier.create(depositScannerService.scanChainForDeposits(nullId)).verifyComplete();
  }
}
