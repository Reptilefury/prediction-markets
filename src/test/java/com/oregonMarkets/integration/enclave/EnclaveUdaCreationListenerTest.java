...
class EnclaveUdaCreationListenerTest {
    ...
    @BeforeEach
    void setUp() {
        listener = new EnclaveUdaCreationListener(
            enclaveClient,
            eventPublisher,
            userRepository,
            objectMapper,
    ...
    void onProxyWalletCreated_Success() throws Exception {
        ...
        verify(enclaveClient).createUDA(
            userId.toString(),
            "test@example.com",
            "0x123",
            "0x456"
    ...
    void onProxyWalletCreated_UserNotFound() {
        ...
        // Note: eventPublisher may still be called in error scenarios, so we don't verify no interactions
    }
}